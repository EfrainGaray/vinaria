...
else
{
    ...
    {
        my $defaults = "+timestamp,+pid,+seh,+unwind,+process,+module,+loaddll,+threadname";
        $opt_debugmsg = $defaults . "," . $opt_debugmsg if ($opt_debugmsg);
        $opt_debugmsg = $defaults if (!$opt_debugmsg);
    }
...
{
    my ($env_string)=@_;
    foreach my $var (shellwords($env_string))
    {
...
sub dump_wine_environment($)
{
    ...
    foreach my $name ("CX_ROOT", "CX_BOTTLE", "WINEPREFIX",
                      "CX_WINDOWS_VERSION", "PATH", "LD_LIBRARY_PATH",
...
sub setup_wine_environment()
{
    ...
    my $lib_path   = expand_string($cxconfig->get("Wine", "LibPath", "$ENV{CX_ROOT}/lib"));
    ...
    if ($cxconfig->get("Bottle", "WineArch", "win32") eq "win64")
    {
        ...
        $dll_path = expand_string($cxconfig->get("Wine", "DllPath") ||
                                  join( ":", map { $_ .= "/wine/$host-windows"; }
                                        grep { -d "$_/wine/$host-windows" } split( /:/, $lib_path )))
            . ":" . $dll_path;
    }

    # Add $CX_ROOT/lib/wine to the end of WINEDLLPATH.
    # This is needed for the loader to find ntdll.so when using the dock icon renaming hack (22144) and not using the preloader.
    $dll_path .= ":" . expand_string(join( ":", map { $_ .= "/wine"; }
                                     grep { -d "$_/wine" } split( /:/, $lib_path ))) unless ($cxconfig->get("Wine", "DllPath"));

    my $lib64_path;
    if (($cxconfig->get("Bottle", "WineArch", "win32") eq "win64" or !-e "$ENV{CX_ROOT}/lib/wine/i386-unix/ntdll.so") and -d "$ENV{CX_ROOT}/lib64")
    {
        $lib64_path = "$ENV{CX_ROOT}/lib64";
    }

    # libd3dshared is used regardless of whether D3DMetal is enabled
    my $libd3dshared_path = $lib64_path . "/apple_gptk/external/libd3dshared.dylib";
    if (-f "$libd3dshared_path")
    {
        $ENV{CX_APPLEGPTK_LIBD3DSHARED_PATH} = $libd3dshared_path;
    }

    # If GStreamer plugins are present in the app, set paths (these are the "system" plugins)
    my $gst_plugins_path = $lib64_path . "/gstreamer-1.0";
    if (-d "$gst_plugins_path")
    {
        $ENV{GST_PLUGIN_SYSTEM_PATH} = $gst_plugins_path;

        # Put the GStreamer plugin registry in our Application Support directory.
        $ENV{GST_REGISTRY} = CXBottle::get_user_dir() . "/gstreamer-1.0-registry.x86_64.bin";
    }

    # Fix for .NET 7/8 apps under Rosetta
    ...
    # We don't want a host PYTHONPATH to trickle down to Wine.
    delete $ENV{PYTHONPATH};

    read_env_from_string($opt_env) if (defined $opt_env);

    # Advertise AVX under Rosetta, unless the user has explicitly disabled it.

    if (defined $ENV{ROSETTA_ADVERTISE_AVX})
    {
        # Rosetta doesn't actually take any value of the variable to mean
        # "disable the feature". We have to unset it entirely.
        if ($ENV{ROSETTA_ADVERTISE_AVX} eq "0" or $ENV{ROSETTA_ADVERTISE_AVX} eq "")
        {
            delete $ENV{ROSETTA_ADVERTISE_AVX};
        }
    }
    else
    {
